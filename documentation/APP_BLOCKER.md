...
### PC Debug Preview & Web Testing

To preview the Focus Guard App Lock UI on PC during development:
1. Navigate to **Settings → App Blocker**
2. Open the **DebugFab** (floating wrench button in bottom-right)
3. Click **"Open App Lock Page →"** (launches `/app-lock` in a new tab with current config query params)
4. Or navigate directly to `http://localhost:3000/app-lock` (web platform automatically bypasses native guards and defaults `studyMode` to `all`).

### Offline / Network Error Policy

- **Policy:** If network requests to `/api/flashcards/review` or `/api/kanji/review` fail due to offline/no-internet conditions, Focus Guard gracefully grants access and auto-unlocks (`kind: 'auto-unlocked'`), ensuring users are never stranded without app access when offline.
