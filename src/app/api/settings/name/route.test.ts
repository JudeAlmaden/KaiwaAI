...
describe("/api/settings/name", () => {
  ...
  it("updates name successfully", async () => {
    vi.mocked(getCurrentUser).mockResolvedValue({ id: "user1" } as Partial<User> as never);
    vi.mocked(prisma.user.update).mockResolvedValue({ id: "user1", name: "New Name" } as Partial<User> as never);
  ...
  it("trims whitespace from name", async () => {
    vi.mocked(getCurrentUser).mockResolvedValue({ id: "user1" } as Partial<User> as never);
    vi.mocked(prisma.user.update).mockResolvedValue({ id: "user1", name: "Trimmed" } as Partial<User> as never);
